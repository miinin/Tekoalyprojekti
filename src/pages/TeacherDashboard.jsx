  ...
  return (
    <div className="animate-fade-in" style={{ padding: '3rem 2rem', boxSizing: 'border-box', margin: '0 auto', width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, #f0f4f8 0%, #d9e2ec 100%)' }}>
        ...
        {!sessionCode ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `${leftSidebarOpen ? 'minmax(320px, 1fr)' : '80px'} minmax(400px, 2.5fr) ${rightSidebarOpen ? 'minmax(320px, 1.2fr)' : '80px'}`, gap: '2rem', alignItems: 'start', transition: 'grid-template-columns 0.3s ease-in-out' }}>
                
                {/* Sidebar */}
                <div style={{ position: 'relative', background: 'rgba(255, 255, 255, 0.85)', backdropFilter: 'blur(16px)', padding: leftSidebarOpen ? '5rem 1.5rem 1.5rem 1.5rem' : '6rem 1rem 1.5rem 1rem', borderRadius: '32px', border: '1px solid rgba(255, 255, 255, 0.6)', boxShadow: '0 20px 50px rgba(0,0,0,0.08)', display: 'flex', flexDirection: 'column', gap: leftSidebarOpen ? '2.5rem' : '1.5rem', transition: 'all 0.3s', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'relative', background: 'rgba(255, 255, 255, 0.85)', backdropFilter: 'blur(16px)', padding: rightSidebarOpen ? '5rem 1.5rem 1.5rem 1.5rem' : '6rem 1rem 1.5rem 1rem', borderRadius: '32px', border: '1px solid rgba(255, 255, 255, 0.6)', boxShadow: '0 20px 50px rgba(0,0,0,0.08)', transition: 'all 0.3s', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                                        ...
                                        <ul style={{ paddingLeft: '0', margin: 0, listStyleType: 'none', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                            {categories[activeCategoryIndex].subcategories.map(sub => (
                                                <li key={sub.id} style={{ marginBottom: '0.2rem', padding: '1.2rem', background: 'white', borderRadius: '16px', borderLeft: '4px solid #0ea5e9', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', borderRight: '1px solid #e2e8f0', borderTop: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0' }}>
                                                    <div style={{ color: '#0f172a', fontWeight: '900', fontSize: '1.05rem', marginBottom: '0.8rem', lineHeight: 1.3 }}>
                                                        {getTopic(sub.id) || "Yleinen"}
                                                    </div>
                                                    <div style={{ color: '#64748b', fontSize: '0.85rem' }}>
                                                        <div style={{ marginBottom: '0.4rem', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '1px', fontWeight: 'bold' }}>Kirjaus kartalla</div>
                                                        <div style={{ color: '#0ea5e9', fontWeight: 'bold', display: 'flex', alignItems: 'flex-start', gap: '0.4rem', lineHeight: 1.3 }}>
                                                            <BookOpen size={14} color="#0ea5e9" style={{ marginTop: '2px', flexShrink: 0 }} /> {sub.name}
                                                        </div>
                                                    </div>
                                                </li>
                                            ))}
        ...
        {confirmModal && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 99999 }}>
                <div className="animate-bounce" style={{ background: 'white', padding: 'clamp(1rem, 2vh, 2rem)', borderRadius: '24px', width: '95%', maxWidth: '600px', boxShadow: '0 25px 50px rgba(0,0,0,0.5)', border: `2px solid ${confirmModal.confirmColor || '#3b82f6'}`, display: 'flex', flexDirection: 'column', gap: 'clamp(0.5rem, 1.5vh, 1.2rem)' }}>
                    ...
                    <p style={{ margin: 0, fontSize: 'clamp(0.9rem, 1.8vh, 1.1rem)', color: '#475569', lineHeight: 1.4, whiteSpace: 'pre-wrap' }}>{confirmModal.text}</p>
                    <div style={{ display: 'flex', gap: '1rem', marginTop: 'clamp(0.2rem, 1vh, 0.8rem)' }}>
                        <button onClick={confirmModal.onConfirm} style={{ flex: 1, padding: 'clamp(0.6rem, 1.5vh, 1rem)', background: confirmModal.confirmColor || '#3b82f6', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: 'clamp(0.9rem, 2vh, 1.1rem)', cursor: 'pointer' }}>{confirmModal.confirmText}</button>
        ...
        {showRecovery && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 99999 }}>
                <div className="animate-fade-in" style={{ background: 'white', padding: 'clamp(1rem, 2vh, 2rem)', borderRadius: '24px', width: '95%', maxWidth: '600px', boxShadow: '0 25px 50px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', gap: 'clamp(0.5rem, 1.5vh, 1.2rem)', position: 'relative' }}>
                    ...
                    <h3 style={{ margin: 0, color: '#0f172a', fontSize: 'clamp(1.1rem, 2.5vh, 1.5rem)', display: 'flex', alignItems: 'center', gap: '0.8rem' }}><Lock color="#3b82f6" size={24} /> Etsi liittymiskoodi</h3>
                    <p style={{ margin: 0, color: '#475569', lineHeight: 1.4, fontSize: 'clamp(0.85rem, 1.8vh, 1rem)' }}>Syötä alle oppitunnille asettamasi PIN-koodi, niin etsimme aktiiviset luokkatilat tietokannasta.</p>
